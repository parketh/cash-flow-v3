...
app.prepare().then(() => {
    ...
    server.get('/responses/:id', (request, response) => {
        Form.findById(request.params.id).then(form => {
            response.json(form)
        })
        .catch((error) => {
            console.error(error)
    ...
    server.post('/responses/:id/comps', (request, response, next) => {
        ...
        Form.findById(request.params.id).then(existingForm => {
    ...
    server.post('/feedback/:id', (request, response, next) => {
        ...
        feedback.save().then(savedResponse => {
            response.json(savedResponse)
        })
        .catch((error) => {
            console.error(error)
