...
const generateModel = (res, readFile, writeFile, response, request) => {
    ...
    const shiftFormula = (formula, colDisp, rowDisp) => {
        ...
        const formulaRegex = /('?[a-zA-Z0-9\s\[\]\.]{1,99})?'?!?\$?[a-zA-Z]{1,3}\$?[0-9]{1,7}(:\$?[a-zA-Z]{1,3}\$?[0-9]{1,7})?/g
    ...
    const updateSheet = (workbook, sheetName, startCol, startRow, dispCol = 0, dispRow = 0, nonce = 0, array = []) => {
        ...
        if (dispCol !== 0) {
            if (nonce === 0) {
                ...
                for (let colNum = rng.e.c; colNum >= startCol - 1; colNum--) {
                    ...
                    for (let rowNum = startRow - 1; rowNum <= rng.e.r; rowNum++) {
                        ...
                        if (cellOld === undefined) {
                            ...
                            col.push({sheet: sheetName, col: colNum, row: rowNum, type: "formula", value: shiftFormula(cellOld.f, dispCol, 0)})
                        } else {                
                ...
                for (let colNum = 0; colNum < array.length - 1; colNum++) {
                    array[colNum].forEach(row => {
                        row.col = row.col + 1
                        row.value = row.type === 'formula'
                                ? shiftFormula(row.value, dispCol, 0)
                                : row.value
                    })
                }
            }
            ...
            const fillContentsAndStyles = (c, r, d) => {
                const styles = ['bold', 'italic', 'underline', 'strikethrough', 'fontSize', 'fontFamily', 'fontColor', 'horizontalAlignment', 'verticalAlignment', 'wrapText', 'shrinkToFit', 'textDirection', 'textRotation', 'verticalText', 'fill', 'border', 'numberFormat']

                const cellNew = workbook.sheet(sheetName).column(c + startCol - 1).cell(r + startRow - 1)
                const cellOld = workbook.sheet(sheetName).column(c + startCol - 1 - d).cell(r + startRow - 1)
                ...
                if (c === array.length) {
                    workbook.sheet(sheetName).column(array.length + startCol).cell(r + startRow - 1).value('')
                }
            }
        ...
        if (dispRow !== 0) {
            ...
            if (nonce === 0) {
                for (let rowNum = rng.e.r; rowNum >= startRow - 1; rowNum--) {
                    ...
                    for (let colNum = startCol - 1; colNum <= rng.e.c; colNum++) {
                        ...
                        if (cellOld === undefined) {
                            ...
                            row.push({sheet: sheetName, col: colNum, row: rowNum, type: "formula", value: shiftFormula(cellOld.f, 0, dispRow)})
                        } else {                
                ...
                for (let rowNum = 0; rowNum < array.length - 1; rowNum++) {
                    array[rowNum].forEach(col => {
                        col.row = col.row + 1
                        col.value = col.type === 'formula'
                                ? shiftFormula(col.value, 0, dispRow)
                                : col.value
                    })
                }
            }
            ...
            const fillContentsAndStyles = (c, r, d) => {
                ...
                const cellNew = workbook.sheet(sheetName).column(c + startCol - 1).cell(r + startRow - 1)
                const cellOld = workbook.sheet(sheetName).column(c + startCol - 1).cell(r + startRow - 1 - d)
                ...
                if (r === array.length) {
                    workbook.sheet(sheetName).column(c + startCol - 1).cell(array.length + startRow).value('')
                }
            }
    ...
    percentNames.forEach((name) => {
        responses[name] = responses[name] === null
                        ? responses[name]
                        : Number(responses[name]) / 100
    })

    // Map useCiqForecast to 'Yes' or 'No' value
    responses.useCiqForecast = responses.useCiqForecast === "Specify own forecast" 
                             ? "No" 
                             : "Yes"

    // Map convMode to 'H' or 'C' values
    responses.convMode = responses.convMode === "Historical"
                       ? "H"
                       : "C"
    ...
    XlsxPopulate.fromFileAsync(readFile)
        .then(workbook => {        
            ...
            const named_ranges_1 = ['ciqId', 'curr', 'restatementType', 'filingMode', 'convMode', 'useCiqForecast', 'g', 'rf', 'erp', 'preTaxRd', 'debtRatio', 't', 'rOverride', 'gearingMeasure', 'index', 'indexId']
            ...
            if (forecastPeriods > 3) {
                for (let i = 0; i < forecastPeriods - 3; i++) {
                    ...
                    forecastArray = updateSheet(workbook, 'Forecasts', 8, 1, dispCol = 1, dispRow = 0, i, forecastArray)
            ...
            for (let i = 0; i < responses.comps.length; i++) {
                workbook.sheet('Beta').column(3).cell(48 + i).value(responses.comps[i].ciqId)
                workbook.sheet('Comps').column(3).cell(36 + i).value(responses.comps[i].ciqId)
            }
